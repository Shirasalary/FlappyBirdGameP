...
public class GameScene extends JPanel {
    ...
    public GameScene(int x, int y, int width, int height) {
        ...
        this.setLayout(null);
        this.type =Constants.SCENE_PANEL;
              ...
              if (!this.allObstacles.isEmpty()){
                 for (int i =0; i<this.allObstacles.size();i++){
                     ...
                     if (this.bird.isCollision(up,down))
                     {
                         this.bird.killBird();
                         stopObstacles();
                          this.type =Constants.MES_SCENE_PANEL;
                          repaint();
                     }
                 }
              }
    ...
    public void paintComponent(Graphics graphics)
    {
        ...
        if (this.type ==Constants.MES_SCENE_PANEL){
            this.add(new Message(this.getWidth()/2 - Constants.MES_WINDOW_WIDTH/2,
                    this.getHeight()/4));
        }
    }
}
